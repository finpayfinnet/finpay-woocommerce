...
// extends WC_Gateway_ 
{
  ...
  public function getPluginOptions($plugin_id = 'finpay')
  {
    ...
    try {
      ...
      wc_get_logger()->error('POST NOTIFICATION: Fail to getPluginOptions');
    };
  ...
  public function handleFinpayNotificationRequest()
  {
    ...
    if (empty($sanitized['order']['id']) && empty($sanitizedPost['id']) && empty($sanitized['id']) && empty($sanitizedPost['response'])) {
      ...
      // @TAG: order-id-suffix-handling
      $raw_notification['order']['id'] =
        WC_Finpay_Utils::check_and_restore_original_order_id($raw_notification['order']['id']);
      // Get WooCommerce order
      $wcorder = wc_get_order($raw_notification['order']['id']);
      // exit if the order id doesn't exist in WooCommerce dashboard
      if (!$wcorder) {
        WC_Finpay_Logger::log('Can\'t find order id' . $raw_notification['order']['id'] . ' on WooCommerce dashboard', 'finpay-error');
        exit;
      }
      ...
      if ($raw_notification['result']['payment']['status'] == 'PAID') {
        ...
        if (wc_get_order($order_id) != false) {
          // @TODO: rename this hook to use snake_case format
          do_action("finpay-handle-valid-notification", $raw_notification, $plugin_id);
        }
      }
      exit;
    }
  ...
  public function handlefinpayValidNotificationRequest($finpay_notification, $plugin_id = 'finpay')
  {
    ...
    if ($finpay_notification->result->payment->status == 'PAID') {
      ...
      // }
      $order->payment_complete($finpay_notification->order->id);
      $order->add_order_note(__('finpay payment completed: ' . $finpay_notification->result->payment->statusDesc . '. finpay-' . $finpay_notification->sourceOfFunds->type, 'finpay-woocommerce'));
      // allow merchant-defined custom action function to perform action on $order
      do_action(
        'finpay_after_notification_payment_complete',
        $order,
        $finpay_notification
      );
      ...
      $plugin_options = $this->getPluginOptions($plugin_id);
      if (
        array_key_exists('custom_payment_complete_status', $plugin_options)
